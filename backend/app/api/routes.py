...
async def trigger_run(request: Request, _: None = Depends(_require_admin)):
    ...
    task.add_done_callback(lambda _done: _active_runs.pop(str(task_id), None))
...
async def get_soul_prompt(_: None = Depends(_require_admin)):
    if not DEFAULT_SOUL_PATH.exists():
        return {"content": ""}
    return {"content": DEFAULT_SOUL_PATH.read_text(encoding="utf-8")}
